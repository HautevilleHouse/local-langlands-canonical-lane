...
In-paper anchor: `paper/LOCAL_LANGLANDS_PREPRINT.md` (`LLG_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of admissible local packets across Weil-Deligne parameters, inertial types, and inner-form packet data through a multi-lane local correspondence super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`LLG_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure packet towers are precompact and local correspondence windows do not collapse.
This is the first-failure capture contribution to `M_LLG`.
...
If near-failure extraction and no-Zeno spacing hold, then `LLG_G3` prevents hidden drift before rigidity is applied.
