...
In-paper anchor: `paper/LOCAL_LANGLANDS_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of admissible local packets across Weil-Deligne parameters, inertial types, and inner-form packet data through a multi-lane local correspondence super-architecture`.
...
2. the endpoint-transfer lock `packet_local` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_LLG` remains positive.
...
Payload: verify `packet_local` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_LLG` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `packet_local`
